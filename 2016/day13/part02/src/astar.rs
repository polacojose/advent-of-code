...
use crate::office::{is_wall, MAX_DISTANCE, OFFICE_DESIGNERS_FAVORITE_NUMBER};
...
fn neighbors_office<HF>(node: &AStarVector, heuristic_function: &HF) -> Vec<AStarVector>
where
    ...
    HF: Fn(&AStarVector) -> f64,
{
    ...
    while let Some(current) = open_set.pop_first() {
        ...
        for neighbor in neighbors_office(&current, &heuristic_function) {
            let tentative_g_score = g_score.get(&current).unwrap() + distance(&current, &neighbor);
            if tentative_g_score <= MAX_DISTANCE
                && tentative_g_score < *g_score.get(&neighbor).unwrap_or(&f64::INFINITY)
            {
