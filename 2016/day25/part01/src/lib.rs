use std::{collections::HashMap, fs, ops::ControlFlow, str::FromStr, sync::RwLock};
...
impl Instruction {
    pub fn execute(&self) -> Result<(), ()> {
        match &self {
            Instruction::cpy { src, dest_register } => {
                if let ControlFlow::Break(_) = Self::process_multiply() {
                    return Ok(());
                }
            ...
            Instruction::jnz { src, offset } => {
                let source_value = Self::register_source_value(src);
                let offset_value = Self::register_source_value(offset);
            ...
            Instruction::out { src } => {
                let source_value = Self::register_source_value(src);
                {
                    ...
                    let clock = *REGISTERS.read().unwrap().get("clock").unwrap();
                    if clock == source_value as usize {
                        return Err(());
                    }
                }
                print!("{source_value}");
                *REGISTERS.write().unwrap().get_mut("clock").unwrap() = source_value as usize;
                Self::increment_instruction_pointer();
                Ok(())
            }
        }
    ...
    fn process_multiply() -> ControlFlow<()> {
...
pub fn set_start_registers(a: usize) {
    ...
    hash_map.insert("clock".to_string(), 2);
    *REGISTERS.write().unwrap() = hash_map;
}

lazy_static! {
    pub static ref REGISTERS: RwLock<HashMap<String, usize>> = RwLock::new(Default::default());
