...
impl VirusCarrier {
    fn progress(&mut self, node_map: &mut NodeMap) {
        ...
        match cur_node.state {
            ...
            NodeState::Flagged => {
                self.vector.heading = self.vector.heading.rotate(2);
            }
        };

        cur_node.state = cur_node.state.advanced_state();
        node_map.set_node(cur_node);

        self.vector.step();
        node_map.get_node(self.vector.position);
    }
}
...
impl NodeMap {
    fn print_nodes(&mut self, marked_pos: (isize, isize)) {
        for y in self.min..self.max {
            for x in self.min..self.max {
                let marker = match self.get_node((x, y)).state {
                    NodeState::Clean => ".",
                    NodeState::Infected => "#",
...
fn main() {
    ...
    for _ in 0..10000000 {
        ...
        //println!("Virus Pos: {:?}", virus.vector.position);
        //node_map.print_nodes(virus.vector.position);
