...
pub fn get_dense_hash(input: impl Into<String>, sparsh_hash_size: usize) -> String {
    let mut lengths: Vec<u8> = input.into().into_bytes();
    for s in [17, 31, 73, 47, 23].into_iter() {
        lengths.push(s);
    }

    let mut sparse_hash = get_sparse_hash(sparsh_hash_size);
...
pub fn get_sparse_hash(size: usize) -> VecDeque<usize> {
