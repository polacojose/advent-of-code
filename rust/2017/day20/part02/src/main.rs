...
fn update_particles(particles: &mut Vec<(usize, Particle)>) {
    for p in particles.iter_mut() {
        p.1.velocity += p.1.acceleration;
    }
    for p in particles.iter_mut() {
        p.1.position += p.1.velocity;
    }
}
...
fn collide() {
    ...
    for _ in 0..100000 {
        ...
        particles
            .sort_unstable_by_key(|(_, item)| (item.position.0, item.position.1, item.position.2));

        let mut i = 1;
        while i < particles.len() {
            let position = particles.get(i).unwrap().1.position;
    ...
    // let mut particles = particles.into_iter().collect::<Vec<_>>();
