...
use part02::game::{GameState, INITIAL_GAME_STATE};
...
async fn main() {
    ...
    loop {
        ...
        for state in state_queue.drain() {
            join_set.spawn(async move { state.get_next_valid_states() });
        }

        while let Some(Ok(next_possible_states)) = join_set.join_next().await {
