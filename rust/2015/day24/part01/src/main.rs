...
fn main() {
    ...
    for first_length in 1..=max_length {
        ...
        for first_set in first_combinations {
            ...
            let mut found = false;
            for second_length in 1..new_available_items.len() {
                ...
                for second_set in second_combinations {
                    *MINIMUM_LENGTH.write().unwrap() = first_set.len();
                    ...
                    let first_qe = first_set.iter().map(|x| *x).reduce(|a, b| a * b).unwrap();

                    *MINIMUM_QUANTIUM_ENTANGLEMENT.write().unwrap() = first_qe;
                    println!("qe: {}", *MINIMUM_QUANTIUM_ENTANGLEMENT.read().unwrap());
                    let third_set = available_items
                        .clone()
                        .into_iter()
                        .filter(|x| !first_set.contains(x) && !second_set.contains(x))
                        .collect::<Vec<u64>>();
                    println!(
                        "1: {:?} = {:.3e}\n2: {:?} = {:.3e}\n3: {:?} = {:.3e}",
                        first_set,
