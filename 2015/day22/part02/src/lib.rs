...
impl SpellType {
    ...
    pub fn end(&mut self, source: &mut Stats, _target: &mut Stats) {
        match self {
            SpellType::Shield {
                cost: _,
                armor,
                ...
                assert!(*turns == 0);
                source.armor = source.armor.saturating_sub(*armor);
                *turns -= 1;
            }
            _ => {}
        }
...
pub fn fight(
    ...
    while player_stats.hp > 0 && boss_stats.hp > 0 {
        ...
        player_stats.hp = player_stats.hp.saturating_sub(1);
        if player_stats.hp <= 0 {
            return Err(Error::PlayerDied);
        }
